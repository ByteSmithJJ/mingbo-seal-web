...
async function handleRouteGuard(
  ...
  // 初始化失败后，登录页和500页始终放行；根路径重定向到登录页以便重新认证
  if (routeInitFailed) {
    if (to.path === RoutesAlias.Login || to.name === 'Exception500') {
      next()
      return
    }
    if (to.path === '/') {
...
function handleLoginStatus(
  ...
  // 登录页和静态路由始终放行（无需登录即可访问）
  if (to.path === RoutesAlias.Login || isStaticRoute(to.path)) {
    return true
  }

  // 已登录且 token 有效，放行
  if (userStore.isLogin && !isTokenExpired(userStore.accessToken)) {
    return true
  }

  // token 过期：提示用户并跳转登录页
  if (userStore.isLogin) {
    ElMessage.warning($t('httpMsg.unauthorized'))
  }
