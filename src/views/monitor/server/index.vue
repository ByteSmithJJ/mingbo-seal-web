...
const memData = computed(() => {
  ...
  if (!mem || !jvm) return []
  return [
    { label: '总内存', mem: `${mem.total}G`, jvm: `${jvm.total}M`, danger: false },
    { label: '已用内存', mem: `${mem.used}G`, jvm: `${jvm.used}M`, danger: false },
    { label: '剩余内存', mem: `${mem.free}G`, jvm: `${jvm.free}M`, danger: false },
    {
      label: '使用率',
      mem: `${mem.usage}%`,
      jvm: `${jvm.usage}%`,
...
const jvmData = computed(() => {
  ...
  return [
    ...
    { label: '安装路径', value: jvm.home, label2: null, value2: null },
    { label: '项目路径', value: sys?.userDir ?? '-', label2: null, value2: null },
    { label: '运行参数', value: jvm.inputArgs, label2: null, value2: null }
  ]
})
