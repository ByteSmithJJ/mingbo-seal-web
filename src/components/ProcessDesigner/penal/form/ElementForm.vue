    ...
    <el-form size="small" label-width="90px" @submit.prevent>
      <el-form-item label="表单" prop="formKey">
        <el-select v-model="formKey" placeholder="请选择表单" @change="updateElementFormKey" clearable>
          <el-option v-for="item in formOptions" :key="item.templateId" :label="`${item.templateName} (v${item.version})`" :value="`key_${item.templateId}`" />
        </el-select>
      </el-form-item>
...
<script lang="ts">
import { fetchTemplateList } from '@/api/form'
...
export default {
  ...
  methods: {
    /** 查询表单列表 */
    getFormList() {
      fetchTemplateList({ pageNum: 1, pageSize: 200 }).then((res: any) => {
        const data = res.data || res
        this.formOptions = (data as any).rows || data || []
      })
    },
    resetFormList() {
